...
class MarketCubit extends Cubit<MarketState> {
  ...
  MarketCubit({required this.repo, this.perPage = 50}) : super(MarketInitial());
  ...
  Future<void> loadInitial() async {
    ...
    try {
      ...
      emit(MarketLoaded(coins: coins, hasReachedEnd: hasReachedEnd, page: page));
    } catch (e) {
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      final newCoins = await repo.fetchMarketCoins(page: nextPage, perPage: perPage);
      _isFetching = false;

      if (current is MarketLoaded) {
        final all = [...current.coins, ...newCoins];
        final hasReachedEnd = newCoins.length < perPage;
        emit(MarketLoaded(coins: all, hasReachedEnd: hasReachedEnd, page: nextPage));
      } else {
        final hasReachedEnd = newCoins.length < perPage;
        emit(MarketLoaded(coins: newCoins, hasReachedEnd: hasReachedEnd, page: nextPage));
      }
    } catch (e) {
      _isFetching = false;
      emit(MarketError(e.toString()));
